...
def test_token_manager_reads_from_cache(
...
def test_token_manager_detects_expired_token(
    token_manager: TokenManager, cache_file: Path
) -> None:
...
def test_token_manager_fallback_to_config_token(
    token_manager: TokenManager, cache_file: Path
) -> None:
...
def test_token_manager_fallback_to_env_var(
    token_manager: TokenManager, cache_file: Path
) -> None:
    cache_file.unlink(missing_ok=True)
...
def test_token_manager_save_token(
    token_manager: TokenManager, cache_file: Path
) -> None:
...
def test_token_manager_get_status(
    token_manager: TokenManager, cache_file: Path
) -> None:
...
def test_neodata_client_query_returns_none_when_no_token(
    cache_file: Path,
) -> None:
    with patch("backend.collectors.neodata_client._TOKEN_FILE", cache_file):
...
def test_neodata_client_query_makes_correct_post(
    cache_file: Path,
) -> None:
    ...
    with (
        ...
        client = NeoDataClient(endpoint="https://example.com/api")
        result = client.query("贵州茅台股价", data_type="api")
...
def test_neodata_client_query_retries_on_auth_error(
    cache_file: Path,
) -> None:
...
def test_neodata_client_query_returns_none_on_retry_failure(
    cache_file: Path,
) -> None:
    _write_cache(cache_file, "bad_tok", int(time.time()))
...
def test_token_manager_valid_jwt_not_expired(
    token_manager: TokenManager, cache_file: Path
) -> None:
...
def test_token_manager_expired_jwt_returns_none(
    token_manager: TokenManager, cache_file: Path
) -> None:
...
def test_token_manager_non_jwt_uses_ttl(
    token_manager: TokenManager, cache_file: Path
) -> None:
...
def test_token_manager_non_jwt_expired_by_ttl(
    token_manager: TokenManager, cache_file: Path
) -> None:
...
def test_token_manager_malformed_jwt_falls_back_to_ttl(
    token_manager: TokenManager, cache_file: Path
) -> None:
...
def test_token_manager_jwt_status_shows_exp(
    token_manager: TokenManager, cache_file: Path
) -> None:
