...
@pytest.fixture
def westock_service():
    return CollectionService(providers={"structured": [WestockMockProvider()], "news": []})
...
async def test_collect_dividend_persists(westock_service):
    ...
    assert len(result) == 1
    with get_db() as conn:
        rows = conn.execute("SELECT * FROM dividends WHERE symbol = 'sh600519'").fetchall()
    assert len(rows) == 1
    row = dict(rows[0])
...
async def test_collect_reserve_persists(westock_service):
    ...
    assert result is not None
    with get_db() as conn:
        rows = conn.execute("SELECT * FROM profit_forecasts WHERE symbol = 'sh600519'").fetchall()
    assert len(rows) == 1
    row = dict(rows[0])
...
async def test_collect_reserve_missing_type_defaults_to_unknown():
    ...
    with get_db() as conn:
        rows = conn.execute("SELECT forecast_type FROM profit_forecasts WHERE symbol = 'sh600519'").fetchall()
...
async def test_collect_shareholder_persists_both_tables(westock_service):
    ...
    assert result is not None
    with get_db() as conn:
        top = conn.execute("SELECT * FROM shareholders WHERE symbol = 'sh600519'").fetchall()
        hist = conn.execute("SELECT * FROM shareholder_count_history WHERE symbol = 'sh600519'").fetchall()
    assert len(top) == 1
    assert len(hist) == 1
...
async def test_collect_intraday_persists_minute_klines(westock_service):
    ...
    assert result is not None
    with get_db() as conn:
        rows = conn.execute("SELECT * FROM minute_klines WHERE symbol = 'sh600519'").fetchall()
    assert len(rows) == 1
    row = dict(rows[0])
