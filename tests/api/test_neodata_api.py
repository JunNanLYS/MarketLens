...
def test_get_token_status(test_client: TestClient, mock_client: MagicMock) -> None:
...
def test_save_token_success(test_client: TestClient, mock_client: MagicMock) -> None:
    resp = test_client.post("/api/v1/neodata/token", json={"token": "my_secret_token"})
    assert resp.status_code == 200
    data = resp.json()
...
def test_save_token_empty_returns_400(
